...
BEGIN
  ...
  scored_results AS (
    SELECT 
      ...
      CASE
        -- Exact email match
        WHEN LOWER(c.email) = (SELECT term FROM search_terms) THEN 100
        -- Exact name match
        WHEN LOWER(c.name) = (SELECT term FROM search_terms) THEN 95
        WHEN LOWER(c.display_name) = (SELECT term FROM search_terms) THEN 95
        -- Exact first or last name match
        WHEN LOWER(c.first_name) = (SELECT term FROM search_terms) THEN 90
        WHEN LOWER(c.last_name) = (SELECT term FROM search_terms) THEN 90
        -- Starts with match
        WHEN LOWER(c.email) LIKE (SELECT term FROM search_terms) || '%' THEN 85
        WHEN LOWER(c.name) LIKE (SELECT term FROM search_terms) || '%' THEN 80
        WHEN LOWER(c.first_name) LIKE (SELECT term FROM search_terms) || '%' THEN 75
        WHEN LOWER(c.last_name) LIKE (SELECT term FROM search_terms) || '%' THEN 75
        -- Contains match
        WHEN LOWER(c.email) LIKE '%' || (SELECT term FROM search_terms) || '%' THEN 70
        WHEN LOWER(c.name) LIKE '%' || (SELECT term FROM search_terms) || '%' THEN 65
        WHEN LOWER(c.company) LIKE '%' || (SELECT term FROM search_terms) || '%' THEN 60
        -- Alias match
        WHEN EXISTS (
